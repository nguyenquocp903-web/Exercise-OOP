public class BankAccount
{
    ...
    public bool Withdraw(decimal amount, string inputPin)
    {
        if (IsLocked)
        {
            Console.WriteLine("Error: Account is locked due to multiple failed PIN\nattempts.");
            return false;
        }
        ...
        if (inputPin != _pin)
        {
            ...
            {
                IsLocked = true;
                Console.WriteLine("Error: Invalid PIN code. Account has been LOCKED for\nsecurity!");
            }
            else
            {
        ...
        _balance -= amount;
        _failedAttempts = 0; // Reset counter upon successful authentication
        return true;
    }
    ...
    {
        if(IsLocked || currentPin != _pin || string.IsNullOrWhiteSpace(newPin))
        {
            return false;
        } return true;
    }
